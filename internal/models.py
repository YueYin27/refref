...
from internal import coord, image, math, ray_samplers, ref_utils, render, stepfun, train_utils
...
class Model(nn.Module):
    ...
    """
        ...
        if self.near_anneal_rate is None:
            init_s_near = 0.  # initialize the near bound to 0
        else:
            init_s_near = np.clip(1 - train_frac / self.near_anneal_rate, 0,
                                  self.near_anneal_init)  # anneal in the near bound
        init_s_far = 1.
        sdist = torch.cat([
            torch.full_like(batch['near'], init_s_near),
            torch.full_like(batch['far'], init_s_far)
        ], dim=-1)  # the distance intervals for each ray, [32768, 2]
        ...
        for i_level in range(self.num_levels):
            ...
            tdist = s_to_t(sdist)  # [8192, 65]

            # Initialize the origins and directions for each ray
            ...
            ray_results = mlp(
                rand,
                means, stds,
                viewdirs=ray_samples.directions if self.use_viewdirs else None,
                refdirs=refdirs,
        ...
        return renderings, ray_history, rfls, ray_results, ray_samples
...
class MLP(nn.Module):
    ...
    enable_pred_kappa: bool = False  # If True compute predicted kappa.
        ...
        if self.disable_rgb:
            ...
            if viewdirs is not None:
                ...
                if self.use_reflections:
                    ...
                    # refdirs = ref_utils.reflect(-viewdirs, normals_to_use)
                ...
                if self.use_n_dot_v:
                    dotprod = torch.sum(
                        normals_to_use * viewdirs, dim=-1, keepdim=True)
                    x.append(dotprod)
