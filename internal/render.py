...
def cast_rays_ref(tdist, origins, directions, cam_dirs, radii, rand=True, n=7, m=3, std_scale=0.5, **kwargs):
    """Cast rays (cone- or cylinder-shaped) and featurize sections of it.
  ...
  """
    ...
    if Backend.get_name() == 'dpcpp':
        ...
        # # just use directions to be the third vector of the orthonormal basis,
        # # while the cross section of cone is parallel to the image plane
        # basis_matrix = torch.stack([ortho1, ortho2, directions], dim=-1)  # [8192, 3, 3]
        # means = math.matmul(means, basis_matrix[..., None, :, :].transpose(-1, -2))
        # means = means + origins[..., None, None, :]  # [8192, 64, 6, 3]
        # # import trimesh
        # # trimesh.Trimesh(means.reshape(-1, 3).detach().cpu().numpy()).export("test.ply", "ply")

        # use origins_ref and directions_ref instead
        # directions_ref = directions.unsqueeze(1).expand(-1, means.shape[1], -1)  # [8192, 64, 3]
        # origins_ref = origins.unsqueeze(1).expand(-1, means.shape[1], -1)  # [8192, 64, 3]
        rand_vec = torch.randn_like(directions)
        ortho1 = F.normalize(torch.cross(directions, rand_vec, dim=-1), dim=-1)
        ortho2 = F.normalize(torch.cross(directions, ortho1, dim=-1), dim=-1)
        basis_matrix = torch.stack([ortho1, ortho2, directions], dim=-1)
        means = math.matmul(means, basis_matrix.transpose(-1, -2))
        means = means + origins[..., None, :]  # [8192, 64, 6, 3]

    return means, stds, t


def compute_alpha_weights_ref(density, tdist, dirs, opaque_background=False):
    """Helper function for computing alpha compositing weights."""
    t_delta = tdist[..., 1:] - tdist[..., :-1]
    delta = t_delta * torch.norm(dirs, dim=-1)
...
def volumetric_rendering(rgbs,
  ...
  Args:
    ...
    depth_method: str, the method to compute the depth, either 'expected' or 'median'.
  ...
  """
    ...
    t_mids = 0.5 * (tdist[..., :-1] + tdist[..., 1:])  # The midpoints of the t-intervals
    ...
    if depth_method == "expected":
        depth = (
            torch.clip(
                ...
                tdist[..., 0], tdist[..., -1]))
    elif depth_method == "median":
        cumulative_weights = torch.cumsum(weights, dim=-1)  # the cumulative sum of weights
        split = torch.ones((*weights.shape[:-1], 1), device=weights.device) * 0.5  # the split point
        median_index = torch.searchsorted(cumulative_weights, split, side="left")  # the index of the split point
        median_index = torch.clamp(median_index, 0, t_mids.shape[-1] - 1)  # clamp the index to between 0 and the number of samples
        depth = torch.gather(t_mids, dim=-1, index=median_index).squeeze(-1)  # compute the median depth
    else:
        raise ValueError(f"Invalid depth_method: {depth_method}. Choose 'expected' or 'median'.")
